...
import { verifyJobSeller } from "@/lib/dispute-access";
...
export async function POST(request: NextRequest) {
  ...
  // On-chain access check: verify user is the job seller
  const accessCheck = await verifyJobSeller(auth.address, jobId);
  if (accessCheck) {
    return NextResponse.json(
  ...
  // 4. Pre-validate all files BEFORE uploading any
  ...
  for (const file of files) {
    ...
    const storagePath = `deliveries/${jobId}/${uuid}-${safeName}`;
  ...
  const deliveryRef = db.collection("deliveries").doc(jobId).collection("files");
